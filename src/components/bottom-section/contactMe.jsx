  ...
  return (
    ...
    <form className='contact-form' ref={form} onSubmit={sendEmail}>
        ...
        <div className='inputs'>
            <input required className='input-text' type="text" name="user_name" placeholder='Nom ou entreprise' />
            <input required onChange={handleEmailChange} className='input-text' value={email} type="text" name="user_email" placeholder='Email'/>
            {emailError && <span className="email-error">{emailError}</span>}
            <textarea required className='input-text textArea' type="text" name="message" placeholder='Message'/>
            
            
            <div className="sending-button" >
                <button className={`svg-button ${submitted, formAlreadySubmitted ? "disabled" : ""} ${active ? 'active' : ''} `} type='submit' disabled={submitted} >
                    <span>ENVOYER</span>
                    <span>
    ...
    {submitted && (
        <Modal message="Le formulaire a été soumis avec succès! ✅" valid={true}/>
      )}
    
    {submittedError && (
        <Modal message="Oups! Erreur lors de la soumission du formulaire. 😓" valid={false}/>
      )}

    {formAlreadySubmitted && (
        <Modal message="Le formulaire a déjà été soumis. Pour me contacter, n'hésitez pas à m'envoyer un email ! 📩" alreadySubmitted={true}/>
      )}

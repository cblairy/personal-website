  ...
  return (
    ...
    <form className='contactForm' ref={form} onSubmit={sendEmail}>
        ...
        <div className='inputs'>
            ...
            <label></label>
            <input required onChange={handleEmailChange} className='inputText' value={email} type="email" name="user_email" placeholder='Email'/>
            {emailError && <span className="emailError">{emailError}</span>}
            <label></label>
            <textarea required className='inputText textArea' type="text" name="message" placeholder='Message'/>
            
            
            <div className="sendingButton" >
                <button className={`svgButton ${submitted, formAlreadySubmitted ? "disabled" : ""} ${active ? 'active' : ''} `} type='submit' disabled={submitted} >
                    <span>ENVOYER</span>
                    <span>
                        <svg className='arrowSvg' version="1.1" viewBox="0 0 66 43" >
